...
describe('Starting tests for angular-app', function() {
  ...
  it('network-name should be blch-evoting@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blch-evoting@0.0.1.bna');
    });
  });
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('PollAsset component should be loadable',() => {
      page.navigateTo('/PollAsset');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('PollAsset');
      });
    });

    it('PollAsset table should have 3 columns',() => {
      page.navigateTo('/PollAsset');
